...
- **`HTTP 400 Malformed publisher model (…expected '<publisher>/<model>')`.**
  Vertex's OpenAI-compatible surface (`…/endpoints/openapi`) addresses a model
  as `<publisher>/<model>`; a bare `gemini-3.8-flash` is refused by the
  endpoint before the model is ever consulted. The provider profile already
  said so in its `modelExample`, which is not where anyone reads it. Now
  `providers.model_for(base_url, model)` qualifies a bare name for Vertex
  (family table: gemini/gemma/imagen → `google`, llama → `meta`, mistral →
  `mistralai`, claude → `anthropic`, …, unknown → `google`), and both request
  paths pass through it - `agent._model_for` and the connection test in
  `main.py`, so the test measures the request the agent will make. An
  already-qualified name and every other provider are untouched.
  `providers.hint` also answers this error in words, and the Vertex profile
  note now states the format.
- **`Vertex JSON 인증에 필요한 google-auth가 설치되지 않았습니다` on an updated
  install.** google-auth has been a pinned dependency since 0.15.14 and every
  release bundle carries it, but `updater.py` cannot replace the interpreter it
  is running on: it stages the new one as `python.new` for the launcher to swap
  in. An install started another way (run.py directly, or a source `.venv`)
  therefore gains new code on old packages, and the person configuring Vertex
  has no way to fix that from the panel. `vertexauth._google_auth()` now
  repairs it once per process with the bundled pip, on the pin read from the
  shipped `requirements.in`, logging what it does; if that fails too, the error
  names the cause (staged `python.new`, a `.venv` install, or an old install)
  and gives the exact `-m pip install` command with this interpreter's path.
...
**+ §1-61 (2026-09-06, 0.14.8) - lorebook folders are folders in RisuAI too**. The report: a folder the agent made showed up in Risu Hina's tree but not in RisuAI after 반영 - RisuAI treats the entry as `mode: none`/normal, and the only way out was RisuAI's own editor plus moving members one by one. Cause: RisuAI recognises a folder ONLY as a lorebook entry with `mode: 'folder'` (its `key` is the id members name in `folder`, its `comment` the name), and neither `propose_lore_add` nor `propose_lore_edit` could set `mode`, while the panel's tree grouped by the members' `folder` value alone - so a "folder" existed here that RisuAI had never been told about. Now: (1) `store.LORE_MODES` / `lore_folder_key` / `lore_folder_entry`; `propose_lore_add(mode=...)` (default normal; `mode="folder"` makes the container - comment is the name, keys the id or a generated one, the proposal names it) and `propose_lore_edit(mode=...)` (`folder` converts an entry, generating an id if its key is empty; the summary says 폴더로 전환); `list_lore` says so. (2) The lorebook tab: 새 폴더 beside 새 항목; a folder row carries ✎ opening the folder's own editor (name, id, member count, 저장 / 일반 항목으로 / 삭제 with the member count in the confirm); an empty folder still has a row; members naming an id no folder entry has get a 폴더 만들기 button on their row that creates the missing container under that id; an entry's editor has 폴더로 전환. plugin_smoke `test_lore_folders`, test_studio `test_lore_folder_entry`.

**+ §1-60 (2026-09-06, 0.14.8) - the review grid no longer jumps on a phone**. The report: scrolling the 검수 grid on the phone, past a point "the next picture jumps to the top". Cause: §1-55 drops a thumbnail's `<img>` when its cell scrolls 600px away and fetches it again on return, and the review grid sizes cells by the picture (`.selgrid .assetpic { aspect-ratio: auto; min-height: 60px }`) - so `loadThumb`'s unload (`clear(mount)`) collapsed the cell to 60px and the reload grew it back. Every unload ABOVE the viewport moved the page by the difference, and iOS Safari has no scroll anchoring to absorb it (the file grids were spared: `workspaceImage` already freezes its box, the selector's own loader did not). Now the picture's natural ratio is pinned on the cell as an inline `aspect-ratio` on load and survives the clear; a cell dropped before its picture ever finished loading freezes at its current size instead. Plugin only; visible after the next release and `+`.
