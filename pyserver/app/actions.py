...
def propose(kind: str, *, chat_key: str, char_key: str, summary: str,
    ...
    args = dict(args or {})
    # Pending cards can outlive a clean workspace reload. Lore row ids are
    # private database ids and may be regenerated by that reload, so retain a
    # semantic locator that approval can use to find the same entry again.
    if kind in ("lore_edit", "lore_delete", "lore_move") and args.get("id"):
        from . import store
        target = store.lore_entry(str(args["id"]))
        if target is not None:
            args["loreTarget"] = {
                "scope": target["scope"], "chatKey": target.get("chatKey"),
                "seq": target["seq"], "entry": target.get("entry") or {},
            }
    aid = uuid.uuid4().hex
    db.execute(
        "INSERT INTO pending_actions(id, session_id, chat_key, char_key, kind, "
        "args_json, summary, status, created_at) VALUES(?,?,?,?,?,?,?,?,?)",
        (aid, session_id, chat_key, char_key, kind, db.js(args), summary,
         PENDING, db.now()),
    )
...
def scope_of(action: dict) -> str:
    ...
    if kind in ("lore_edit", "lore_delete", "lore_move"):
        ...
        if row is None:
            target = action["args"].get("loreTarget") or {}
            scope = target.get("scope")
            return "card" if scope == "global" else ("chat" if scope == "local" else "")
...
def _lore_edit(a: dict) -> str:
    from . import store, textedit
    args = a["args"]
    lore_id = store.resolve_lore_id(a["charKey"], str(args["id"]), args.get("loreTarget"))
    entry = args["entry"]
    patch = args.get("contentReplace")
    if patch:
        current = store.lore_entry(lore_id)
        if current is None:
            raise LookupError("없는 로어북 항목입니다")
        entry = dict(current["entry"] or {})
        entry["content"], _ = textedit.replace_once(
            str(entry.get("content") or ""), str(patch.get("find") or ""),
            str(patch.get("replace") or ""), replace_all=bool(patch.get("replaceAll")))
    store.update_lore(lore_id, entry)
...
def _lore_delete(a: dict) -> str:
    from . import store
    lore_id = store.resolve_lore_id(a["charKey"], str(a["args"]["id"]), a["args"].get("loreTarget"))
    store.delete_lore(lore_id)
...
def _lore_move(a: dict) -> str:
    from . import store
    lore_id = store.resolve_lore_id(a["charKey"], str(a["args"]["id"]), a["args"].get("loreTarget"))
    out = store.move_lore(lore_id, int(a["args"].get("toSeq") or 0))
