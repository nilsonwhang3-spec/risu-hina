...
[sendChat starts: also on reroll, continue, auto-continue]
 2. prompt parts built and CBS-parsed    ← main prompt, description, persona, author's note; lorebook matching (which
                                             entries activate) and CBS of non-depth entries.
 3. Lua onStart                           ← after 2, before the history: variables it sets reach 4-6 (and depth-inserted
 ...
 6. Lua editRequest                       ← the finished request array (OpenAIChat[]). The last Lua touch. Not saved.
    (legacy V2 "request" triggers, if any, run later inside the request call)
[model reply]
...
2. For editdisplay only, legacy V2 triggers of type `display` (deprecated; Lua does not run here). New bots use Lua editDisplay.
...
- **Lua is the supported trigger kind.** V1 triggers (the editor warns "Trigger V1 is deprecated") and V2 block triggers are deprecated: read them in old bots, port them to Lua (table in 'RisuAI Lua 트리거' §0), never add new ones. A `triggerlua` item runs in every mode whatever its `type`; V2-only modes `display` and `request` have no Lua entry point (use editDisplay/editRequest).
...
- `risu-trigger="name"` (and `{{button::label::name}}`, which renders `<button risu-trigger="name">`) runs a **manual trigger** called `name`: for a Lua trigger script it calls the **global Lua function `name(triggerId)`** (each name gets its own engine; the slash command `/trigger name` does the same). In legacy bots it may instead run a deprecated V1/V2 trigger whose `comment` is `name`. No payload is passed, so parameterized actions need one function per value (see the handler families in 'RisuAI 옵션 패널 (슬라이딩 드로어)').
